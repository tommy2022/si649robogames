...
def update():
    try:
        global game, static_text, tree_view, info_view, hints_view, predDict, robo_expiry_sorted, robo_id_buttons
        robo_expiry_sorted.object = getRoboSorted()[0]
        # robo_id_buttons.pop(0)
        # print('poped')
        robo_id_buttons[0] = (getRoboSorted()[1])
        print('id button updated')
        gt = game.getGameTime()
...
def go_clicked(event):
    try:
        global game, network, tree, info, hints, robo_expiry_sorted, robo_id_buttons
        ...
        robo_expiry_sorted.object = getRoboSorted()[0]
        robo_id_buttons.object = getRoboSorted()[1]
...
def getRoboSorted():
    ...
    def on_button_clicked(b):
        # Update and print the global ID
    ...
    box_layout = ipywidgets.Layout(display='flex', flex_flow='row', align_items='stretch', width='800px', margin='5px', padding='0')
    hbox = ipywidgets.HBox(buttons, layout=box_layout)
    print('id buttons created')
    
    return timeSeries, hbox
...
robo_id_buttons = pn.WidgetBox(None)
...
maincol.append(robo_id_buttons)
